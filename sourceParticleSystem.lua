--sperm loading code by adnzzzzZ
...
local image = require(ENGINE_PATH.."/sourceImage")

local sourceParticleSystem = {}

local systems = {}
setmetatable(systems,{__mode = "k"})

function sourceParticleSystem.update(dt)
    for i in pairs(systems) do
        if i:isStopped() then
            systems[i] = nil
        elseif not i:isPaused() then
            i:update(dt)
        end
    end
end

function sourceParticleSystem.getActive()
    local c = 0
    for i in pairs(systems) do
        c = c+1
    end
...
function sourceParticleSystem.new(file, Image)

    local i = image.new(Image)

    local ps = getPS(file,i:getImage())

    function i:draw(sprite)
        local px,py = sprite:getPiv()--sprite:projectPoint(sprite:getPiv())
